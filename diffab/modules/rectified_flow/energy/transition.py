...
from diffab.tools.eval.hydropathy import hydropathy_prob
...
class PositionTransition(nn.Module):
    ...
    def denoise(self, p_t, eps_p, mask_generate, t, energy_grad=None, h3_ranges=None):
        # IMPORTANT:
        ...
        if (energy_grad is not None):
            # 设置梯度缩放范围
            mask_expand = mask_generate[..., None]  # shape: (B, N, 1)
            eps_mask = eps_p * mask_expand  # shape: (B, N, 3)
            step_model = (c0 * c1 * eps_mask).norm(dim=-1)  # 求模长
            model_step = step_model[step_model > 0].mean()  # cdr区的平均更新幅度
            # model_step  = step_model[step_model > 0].median()
            force = -energy_grad
            force_step = force.norm(dim=-1)
            energy_step = force_step[force_step > 0].mean()
            max_energy = 0.6 * model_step
            scale = min(1.0, max_energy / (energy_step + 1e-8))
            force = force * scale
            for i, (start, end) in enumerate(h3_ranges):
                h3_len = end - start  # 当前样本的 CDRH3 长度
                mask_i = mask_generate[i]  # 形状类似：tensor([False, False, True, ...])
                # 统计该样本中所有 True 的数量
                mask_true_count = mask_i.sum().item()
                if mask_true_count != h3_len:
                    all_ok = False
                    raise ValueError(
                        f"[Error] Sample {i}: mask_generate True={mask_true_count}, "
                        f"but H3 length={h3_len}. They must match!"
                    )
            # 更新梯度
            # 1) 全 0 force_full，最终将 force_full 加到 p_next 里
            force_full = torch.zeros_like(p_next)  # shape: (B, N, 3)
            # 2) 依次填充 batch 内每个样本的 CDRH3 力
            for i, (start, end) in enumerate(h3_ranges):
                force_h3 = force[i, start:end, :]  # 这是该样本的 CDRH3 力
                true_indices = mask_generate[i].nonzero(as_tuple=True)[0]  # p_next 中 H3 的位置
                # 将 force_h3 填入 force_full
                force_full[i, true_indices, :] = force_h3
            # 3) 更新 p_next
            p_next = p_next + force_full
            # print("energy_guidancing")
        #####
        return p_next
    
    ##lzh
    def reconstruct_x0(self, p_t, eps_p, t,mask_generate):
        alpha_bar = self.var_sched.alpha_bars[t].view(-1, 1, 1)
        p0 = (p_t - torch.sqrt(1 - alpha_bar) * eps_p) / torch.sqrt(alpha_bar)
        p_next = torch.where(mask_generate[...,None].expand_as(p_t),p0,p_t)
        return p_next
...
class AminoacidCategoricalTransition(nn.Module):
    ...
    def __init__(self, num_steps, num_classes=20, var_sched_opt={}, prior_b=0):
        super().__init__()
        self.num_classes = num_classes
        self.var_sched = VarianceSchedule(num_steps, **var_sched_opt)
        self.prior_b = prior_b # weight of prior distributions
                               # (b=0: uniform distribution; b=1: hydropathy distribution)
        self.hydropathy_prob = torch.tensor(hydropathy_prob)
        self.combined_prob = prior_b*self.hydropathy_prob + (1-prior_b)/num_classes
    ...
    def prior(self, x):
        """
        Args:
            x:      Category LongTensor (N, L).
        Returns:
            x_rand: Randomly sampled LongTensor at timestep T, (N, L).
        """
        self.combined_prob = self.combined_prob.to(x.device)
        theta = torch.tile(self.combined_prob, dims=(x.shape[0],x.shape[1],1))
        x_rand = self._sample(theta)
        return x_rand
    ...
    def posterior(self, x_t, x_0, t):
        ...
        """
        self.combined_prob = self.combined_prob.to(t.device)
        K = self.num_classes
        ...
        theta = (
            ((alpha*c_t) + (1-alpha)*self.combined_prob) *
            ((alpha_bar*c_0) + (1-alpha_bar)*self.combined_prob)
        ) # (N, L, K)
        theta = theta / (theta.sum(dim=-1, keepdim=True) + 1e-8)
        return theta

    def denoise(self, x_t, c_0_pred, mask_generate, t, b=1):
        """
        Args:
            x_t:        (N, L).
