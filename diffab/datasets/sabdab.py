...
FLOWTESTS = {
    '7che_H_L_R', '5w9h_H_I_G', '7d6i_B_C_A', '8ds5_C_B_A', '7chb_H_L_R',
    '5tlk_B_A_X', '7faf_g_f_B', '7ucf_H_L_G', '2r4s_H_L_A', '6al5_H_L_A',
    '4lmq_E_I_F', '4zs6_C_D_B', '4zso_D_C_F', '2hjf_A_B_C', '2yc1_A_B_C',
    '5ec2_A_B_C', '1h0d_B_A_C', '6by2_A_B_C', '7n4i_H_L_C', '7nd9_H_L_B'
}
...
def make_placeholder_template(data):
    ...
    if 'aa' in template:
        ...
        template['aa'][generate_flag] = int(constants.AA.UNK)
    ...
    if 'pos_heavyatom' in template:
        ...
        fake_pos.zero_()
        fake_pos[:, int(constants.BBHeavyAtom.N)] = fake_pos.new_tensor([-1.0, 0.0, 0.0])
        fake_pos[:, int(constants.BBHeavyAtom.CA)] = fake_pos.new_tensor([0.0, 0.0, 0.0])
        fake_pos[:, int(constants.BBHeavyAtom.C)] = fake_pos.new_tensor([0.0, 1.0, 0.0])
...
def generate_name(pdbcode, hchain, lchain, agchains):
    return pdbcode+'_'+nan_to_empty_string(hchain)+'_'+nan_to_empty_string(lchain)+'_'+''.join(agchains)
...
class SAbDabDataset(Dataset):
    ...
    def __init__(
        ...
        processed_dir = './data/processed_RF',
        split = 'train',
        split_seed = 2022,
        transform = None,
        reset = False,
    ...
    def _create_clusters(self):
        ...
        for id in self.db_ids:
            ...
            cdr_seqs = []
            for chain_type, cdr_name in cdr_keys:
                seq = None
                if structure.get(chain_type) and structure[chain_type].get(cdr_name):
                    seq = structure[chain_type][cdr_name]
                if seq and set(seq) != {'-'}:
                    cdr_seqs.append(seq)

            if len(cdr_seqs) > 0:
                concat_seq = ''.join(cdr_seqs)
    ...
    def _load_split(self, split, split_seed):
        ...
        """
        # NOTE change dataset to compare with dymean
        ids_test = [
            entry['id']
            for entry in self.sabdab_entries
            if generate_name(entry['pdbcode'], entry['H_chain'], entry['L_chain'], entry['ag_chains']) in FLOWTESTS
        ]
        ####
        ...
        ids_train_val = [
            entry['id']
            for entry in self.sabdab_entries
            if self.id_to_cluster[entry['id']] not in test_relevant_clusters and generate_name(entry['pdbcode'], entry['H_chain'], entry['L_chain'], entry['ag_chains']) != '3o2d_H_L_A'
        ]
        random.Random(split_seed).shuffle(ids_train_val)
        if split == 'test':
            self.ids_in_split = ids_test
        elif split == 'val':
            self.ids_in_split = ids_train_val[:300]
        else:
            self.ids_in_split = ids_train_val[300:]
    ...
    def __getitem__(self, index):
        ...
        if self.pkl_dict:
            # CHANGE BEGIN
            try:
                template_path = self.pkl_dict.get(id)
            except:
                print(index)
                raise KeyError
            if id in ['7t3d_O_Q_G', '7d3k_H_L_2', '5th9_J_N_C', '7uov_H_L_A', '3csy_E_F_LK']:
                template_path = None
            if template_path:
            ...
            if template_path and os.path.exists(template_path):    
                ...
                template_enable = False
                # logging.warning(f'{id} has a template.')
            else:
                template = None
            
        if template is None:
            template = make_placeholder_template(data)
            template_enable = False

        data['template'] = template
        data['template_enable'] = template_enable
        data['id'] = id
