...
class TaskScanner:
        ...
        try:
            ...
            if info is not None:
                if tag_name == 'MultipleCDRs':
                    ...
                    heavy_chain, light_chain, _ = self._parse_chain_ids_from_name(info)
                    antibody_chains = []
                    for chain_id in (heavy_chain, light_chain):
                        if chain_id is not None and chain_id not in antibody_chains:
                            antibody_chains.append(chain_id)
                    if not antibody_chains and info.get('residue_first') is not None:
                        antibody_chains.append(info['residue_first'][0])
                    info['antibody_chains'] = antibody_chains
    ...
    def _build_multicdr_metadata(self, fpath, info):
        ...
        expected_heavy, expected_light, _ = self._parse_chain_ids_from_name(info)
        antibody_chains = []
        for chain_id in (heavy_chain or expected_heavy, light_chain or expected_light):
            if chain_id is not None and chain_id not in antibody_chains:
                antibody_chains.append(chain_id)
