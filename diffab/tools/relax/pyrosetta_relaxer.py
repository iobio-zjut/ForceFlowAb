...
def is_multicdr_region(flexible_residue_first):
    return (
...
class RelaxRegion(object):
    ...
    def __call__(self, pdb_path, flexible_residue_first, flexible_residue_last, cdrs=None):
        ...
        is_multi_region = is_multicdr_region(flexible_residue_first)
        ...
        if self.subset != 'all':
            if is_multi_region:
                normalized_cdrs = normalize_cdr_order(cdrs)
