...
from diffab.modules.encoders.src.common.rigid_utils import Rigid
from diffab.modules.encoders.src.net.layers import Linear, NodeTransition, EdgeTransition, TorsionAngleHead, BackboneUpdate
...
if __name__ == '__main__':
    ...
    # import torch
    # from src.net.ipa import TranslationIPA
    ...
    N = 1           # batch
    L = 10          # 序列长度
    c_s = 128       # node channel
    c_z = 64        # edge channel

    ipa = TranslationIPA(
        c_s=c_s,
        c_z=c_z,
        coordinate_scaling=10.0,
        no_ipa_blocks=2,
        skip_embed_size=32,
    )
    ...
    # 2. 构造输入：节点和边特征
    # -----------------------------
    node_embed = torch.randn(N, L, c_s)          # 节点特征
    edge_embed = torch.randn(N, L, L, c_z)       # 边特征

    # -----------------------------
    # 3. 构造 batch 字典
    # -----------------------------
    res_mask = torch.ones(N, L)                  # 全部有效
    fixed_mask = torch.zeros(N, L)               # 全部都是可移动残基

    # 构造初始 Rigid：7维 (qx,qy,qz,qw,x,y,z)
    rigids_t = torch.randn(N, L, 7)
    ...
    # 4. 执行 TranslationIPA
    # -----------------------------
    output = ipa(node_embed, edge_embed, batch)

    # -----------------------------
    # 5. 查看输出
    # -----------------------------
    print("输出 keys:", output.keys())
    print("embed shape:", output["embed"].shape)
    print("输出坐标 shape:", output["out_rigids"].to_tensor_7().shape)
    print("psi 角预测 shape:", output["psi"].shape)
