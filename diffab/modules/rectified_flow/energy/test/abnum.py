...
def run_energy_guidance(
    ...
    for i in range(batch_size):
        pdb_dir = os.path.join(save_path, f"batch{batch_id:02d}_sample{i:02d}_step{t:03d}.pdb")
        structure_source = structure_sources[i] if structure_sources is not None else pdb_dir
        chain_map = classify_chains(structure_source)
        chain_map, reordered_model, renum_model = reorder_and_renumber_pdb(structure_source, chain_map)
        cdr_mask, _, _, _, _, cdr_h_lengths, _ = generate_cdr_mask(reordered_model, chain_map)
        coords, atnames, mask_atom = build_madrax_inputs(chain_map, reordered_model, renum_model)
        grad_ca, meta = eg.compute_grad_batch(
        ...
        grads_accum.append(grad_ca)
        if grads_list is not None:
            grads_list.append(grad_ca)
        if meta_list is not None:
            meta_list.append(meta)

        h1_len, h2_len, h3_len = cdr_h_lengths
        h3_start = h1_len + h2_len
        h3_ranges.append((h3_start, h3_start + h3_len))

    grads_batch = torch.cat(grads_accum, dim=0)
