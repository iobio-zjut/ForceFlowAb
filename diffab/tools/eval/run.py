...
from energy import eval_interface_energy


def evaluate(task, args):
    # 523: Keep evaluation steps local to one worker and avoid nesting Ray tasks in the pipeline body.
    funcs = []
...
def evaluate_remote(task, args):
    # 523: Single remote boundary for one evaluation task.
    # Previous structure reference:
    # @ray.remote(num_cpus=1)
    # def evaluate(task, args):
    #     funcs = []
    #     funcs.append(eval_similarity)
...
def main():
    ...
    with shelve.open(db_path) as db:
        ...
        # while True:        
        tasks = scanner.scan()
        futures = [evaluate_remote.remote(t, args) for t in tasks]
        if len(futures) > 0:
...
# python  diffab/tools/eval/run.py  --root  results/codesign_single_H3 --pfx rosetta --no_energy
