...
if __name__ == '__main__':
    ...
    train_iterator = inf_iterator(DataLoader(
        ...
        shuffle=True, ### TODO: change back to TRUE!!!
    ...
    if args.resume is not None or args.finetune is not None:
        ...
        logger.info('Skipping optimizer and scheduler to test learning rate!')
        logger.info('Resuming optimizer states...')
        optimizer.load_state_dict(ckpt['optimizer'])
        logger.info('Resuming scheduler states...')
        scheduler.load_state_dict(ckpt['scheduler'])
        # resume checkpoint  best_val_loss
        if 'best_val_loss' in ckpt:
            best_val_loss = ckpt['best_val_loss']*100
